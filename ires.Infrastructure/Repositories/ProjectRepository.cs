...
namespace ires.Infrastructure.Repositories
{
    public class ProjectRepository(DataContext dataContext, IMapper mapper, ILogService logService, ICurrentUserService _currentUserService) : IProjectService
    {
        ...
        {
            var query = _dataContext.projects.Where(x => x.companyid == _currentUserService.companyid && x.projectypeid == ProjectType.Subdivision
        ...
        public async Task<bool> IsNameUnique(string propertyName, ProjectType projectType)
        {
            return !await _dataContext.projects.Where(x => x.companyid == _currentUserService.companyid
                && x.propertyname == propertyName && x.projectypeid == projectType).AnyAsync();
        }
    }
}
